...
normalized_trend_plot <- ggplot(yearly_avg_data, aes(x = year)) +
  ...
  theme(
    ...
    plot.caption = element_text(size = 18, hjust = 1, margin = margin(t = 10))
  ) +
  labs(
...
# climate_spatial_data, shapefile_Dhaka_boundari, combined_flood_data, NL_data_spatial, water_Dhaka, water_way_Dhaka
reports/plots/dhaka_water_bodies_and_waterways.png

```


```{r}
```notebooks/DataViz.Rmd
# Ensure the required data is loaded:
# climate_spatial_data, shapefile_Dhaka_boundari, combined_flood_data, NL_data_spatial, water_Dhaka, water_way_Dhaka
...
data_2022 <- climate_spatial_data %>% filter(year == 2022)
...
flood_risk_map <- ggplot() +
  ...
  map_theme +
  labs(
    title = "Flood Risk Map of Dhaka (2022)",
    caption = "Source: INFORM Sub-national Risk Index & OpenStreetMap 2022 | Visualization: Bachir Sabo"
  ) +
  coord_sf(datum = NA)

ggsave("reports/plots/dhaka_flood_risk_map_2022.png", flood_risk_map, width = 15, height = 10, dpi = 300)

#------------------------------------------------------------------------------#
# 2. Night Light Intensity Map (2022)
#------------------------------------------------------------------------------#
NL_data_2022 <- NL_data_spatial %>% filter(year == 2022)
NL_data_2022$VIIRS_NTL_LS <- log_scale(NL_data_2022$VIIRS_NTL)
night_light_intensity_map <- ggplot() +
  geom_sf(data = NL_data_2022, aes(fill = VIIRS_NTL_LS), color = NA) +
  scale_fill_viridis_c(
    option = "inferno",
    name = "Night Light Intensity (Log scaled Radiance)",
    guide = guide_colorbar(
      title.position = "top",
      title.hjust = 0.5,
      barwidth = unit(20, "cm"),
  ...
  map_theme +
  labs(
    title = "Night Light Intensity Map of Dhaka (2022)",
    caption = "Source: VIIRS Nighttime Lights | Visualization: Bachir Sabo"
  ) +
  coord_sf(datum = NA)

ggsave("reports/plots/dhaka_LS_night_light_intensity_map_2022.png", night_light_intensity_map, width = 15, height = 10, dpi = 300)
...
flood_extent_comparison_map <- ggplot() +
  ...
  map_theme +
  labs(
    title = "Flood Extent Comparison in Dhaka (2014 vs 2016)",
...
ggsave("reports/plots/dhaka_flood_extent_comparison_2014_2016.png", flood_extent_comparison_map, width = 15, height = 10, dpi = 300)

#------------------------------------------------------------------------------#
# 4. Water Bodies and Waterways Map (2022)
#------------------------------------------------------------------------------#
water_bodies_and_waterways_map <- ggplot() +
  ...
  map_theme +
  labs(
    title = "Water Bodies and Waterways in Dhaka (2022)",
...
ggsave("reports/plots/dhaka_water_bodies_and_waterways.png", water_bodies_and_waterways_map, width = 15, height = 10, dpi = 300)

#------------------------------------------------------------------------------#
# 5. River Discharge Map (2022)
#------------------------------------------------------------------------------#
river_discharge_map <- ggplot() +
  geom_sf(data = shapefile_Dhaka_boundari, fill = NA, color = "black", size = 0.5) +
  geom_sf(data = data_2022, aes(fill = river_discharge), color = NA) +
  scale_fill_viridis_c(
    option = "plasma",
  ...
  map_theme +
  labs(
    title = "River Discharge in Dhaka (2022)",
    caption = "Source: CEMS-GloFAS historical dataset | Visualization: Bachir Sabo"
  ) +
  coord_sf(datum = NA)

ggsave("reports/plots/dhaka_river_discharge_2022.png", river_discharge_map, width = 15, height = 10, dpi = 300)

#------------------------------------------------------------------------------#
# 6. Soil Moisture Map (2022)
#------------------------------------------------------------------------------#
soil_moisture_map <- ggplot() +
  geom_sf(data = shapefile_Dhaka_boundari, fill = NA, color = "black", size = 0.5) +
  geom_sf(data = data_2022, aes(fill = soil_moisture_0_to_10cm_mean), color = NA) +
  scale_fill_viridis_c(
    option = "magma",
  ...
  map_theme +
  labs(
    title = "Soil Moisture in Dhaka (2022)",
    caption = "Source: CEMS-GloFAS historical dataset | Visualization: Bachir Sabo"
  ) +
  coord_sf(datum = NA)

ggsave("reports/plots/dhaka_soil_moisture_2022.png", soil_moisture_map, width = 15, height = 10, dpi = 300)
